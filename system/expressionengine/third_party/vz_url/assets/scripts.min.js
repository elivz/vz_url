...
return;}else{vzUrl.set_status($field,"checking");}if(a){vzUrl.timer=setTimeout(function(){vzUrl.validate($field);},500);}else{vzUrl.validate($field);}},validate:function(c){var b=c.val();
...
return;}var a=b.replace("http","");$.getJSON(EE.BASE+"&callback=?",{caller:"vz_url",url:a},function(d){if(d.original!=b){return;}if((d.original==d.final_url)&&(d.http_code>=200)&&(d.http_code<400)){vzUrl.set_status(c,"valid");
}else{if(d.original!=d.final_url){vzUrl.set_status(c,"redirect",d);}else{vzUrl.set_status(c,"invalid");}}});},set_status:function(d,b,c){d.removeClass("empty checking invalid valid nonlocal redirect");
