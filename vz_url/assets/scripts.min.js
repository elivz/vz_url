jQuery(function(b){var a={regex:new RegExp(/\b((?:[a-z][\w-]+:(?:\/{1,3}|[a-z0-9%])|www\d{0,3}[.]|[a-z0-9.\-]+[.][a-z]{2,4}\/)(?:[^\s()<>]+|\(([^\s()<>]+|(\([^\s()<>]+\)))*\))+(?:\(([^\s()<>]+|(\([^\s()<>]+\)))*\)|[^\s`!()\[\]{};:'".,<>?«»“”‘’]))/i),init:function(c){b(".vz_url_field").each(a.check_field).live("keyup",a.check_field).live("paste",function(d){setTimeout(function(){a.check_field.call(d.target);
...
return;}else{$field.removeClass("empty");}a.timer=setTimeout(function(){a.ajax_call($field);},350);},ajax_call:function(d){var c=d.val();if(!a.regex.test(c)){a.set_status(d,"invalid");
return false;}if(d.hasClass("local")&&c.substr(0,1)!="/"&&c.indexOf(document.domain)==-1){a.set_status(d,"nonlocal");return false;}b.getJSON(EE.BASE+"&callback=?",{caller:"vz_url",url:c},function(e){if(e.original!=c){return;
...
switch(d){case"empty":case"checking":case"valid":break;case"invalid":c.html(vzUrl_settings.errorText);break;case"nonlocal":c.html(vzUrl_settings.nonlocalText);
