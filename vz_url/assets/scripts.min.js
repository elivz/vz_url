var vzUrl={regex:new RegExp("^((https?|ftp)://[\\w\\-_]+(\\.[\\w\\-_]+)+|/)([\\w\\-\\.,@?^=%&amp;:/~\\+#]*[\\w\\-\\@?^=%!&amp;/~\\+#])?$","gi"),init:function(a){$(".vz_url_field").on("keyup paste",function(b){vzUrl.check_field.call(b.target,true);
}).each(vzUrl.check_field);},check_field:function(a){$field=$(this);if(a&&vzUrl.timer){clearTimeout(vzUrl.timer);}if($field.val()==="http://"||$field.val()===""){vzUrl.set_status($field,"empty");
